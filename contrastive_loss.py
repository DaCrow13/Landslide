...
class SupContrastLoss(nn.Module):
    ...
    def forward(self, features, labels=None, mask=None):
        ...
        # Larger epsilon for FP16 stability
        log_prob = logits - torch.log(exp_sum + 1e-4)

        # compute mean of log-likelihood over positive
        mask_pos_counts = mask.sum(1)
        # Guard against division by zero even more robustly
        mask_pos_counts = torch.clamp(mask_pos_counts, min=1.0)
